...
- 심층 콘텐츠 기준: 3000~5000자, h2/h3/blockquote/ul/ol 태그 활용, 내부 링크 1-2개 자동 삽입
- 최소 검증 기준: 2000자 미만 시 생성 실패 처리

### Twitter/X + Bluesky 자동 포스팅 (일일)

X API (twitter-api-v2) + AT Protocol (@atproto/api)를 사용하여 매일 자동으로 Twitter와 Bluesky에 동시 게시합니다.
오늘 생성된 블로그 포스트가 있으면 블로그 포스트, 없으면 랜덤 운세 포스트를 게시합니다.
...
- `scripts/post-to-bluesky.ts` - Bluesky 게시 스크립트
- `scripts/twitter-post-state.json` - Twitter 게시 상태 추적 (lastPostDate, postedSlugs)
- `scripts/bsky-post-state.json` - Bluesky 게시 상태 추적 (lastPostDate, postedSlugs)
- `.github/workflows/daily-twitter-post.yml` - 매일 자동 실행 (Twitter + Bluesky)
...
**GitHub Actions 설정:**
- GitHub Secrets 필요: `X_CONSUMER_KEY`, `X_SECRET_KEY`, `X_ACCESS_TOKEN`, `X_ACCESS_TOKEN_SECRET`, `BLUESKY_HANDLE`, `BLUESKY_APP_PASSWORD`
- 매일 오전 8시(KST) 자동 실행 (블로그 생성 2시간 후)
- 상태 파일 변경은 main에 직접 커밋 (PR 불필요)
- 동시성 그룹: `social-posting` (content-generation과 별도)
...
| 매일 08:00 KST | daily-social-post | 블로그/운세 Twitter+Bluesky 자동 게시 + 상태 커밋 | 중복 이슈 + 자동 닫기 |
